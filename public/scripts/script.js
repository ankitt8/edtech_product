...
function bindEvents() {
  ...
  document
    .querySelector('#submitTestModalClose')
...
function handleFormSubmit(e) {
  ...
  document.getElementById(
    'score'
  ).innerHTML = `Score <span class="score">${score}/${totalQuestions}</span>`;
...
function createTimer(maxTimeInMinutes, formElement) {
  ...
  if (maxTimeInMinutes >= 60)
    hoursRemaining = Math.floor(maxTimeInMinutes / 60);

  var hoursRemainingDisplay =
    hoursRemaining < 10 ? `0${hoursRemaining}` : `${hoursRemaining}`;
  var minutesRemainingDisplay =
    minutesRemaining < 10 ? `0${minutesRemaining}` : `${minutesRemaining}`;
  var secondsRemainingDisplay =
